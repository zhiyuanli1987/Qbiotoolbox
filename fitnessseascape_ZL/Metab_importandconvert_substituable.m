function metabfuns=Metab_importandconvert_substituablev2(species_para)
...
% Define: Growth  rate:
Growth=@(cs,xs)sum(Es_convert.*xs);
...
GC_cb_dim2=@(c_a,D)(Ks(2))/(1/((D/(V_import)-(Es_convert(1)*Es_import(1))/(D+Es_convert(1)*k)*c_a/(c_a+Ks(1)))/((Es_convert(2)*Es_import(2))/(D+Es_convert(2)*k)))-1);
...
GC_cb_dim3=@(c_a,c_b,d)Ks(3)/(((Es_convert(3)*Es_import(3))/(d+Es_convert(3)*k))/(d/(V_import)-(Es_convert(1)*Es_import(1))/(d+Es_convert(1)*k)*c_a/(c_a+Ks(1))-(Es_convert(2)*Es_import(2))/(d+Es_convert(2)*k)*c_b/(c_b+Ks(2)) )-1);
